...
export default function ClientLayout({ children }) {
  ...
  const isHome = pathname === "/"; // Check if current route is homepage

  return (
    <AlertProvider>
      {/* Show header only if NOT home page */}
      {!isHome && (
        <header className="fixed inset-x-0 top-0 z-50 h-16 bg-white shadow-sm border-b border-gray-200 flex items-center justify-between px-8">
          <div className="flex items-center gap-3">
            ...
            <h1 className="text-lg font-semibold text-gray-900">Ticketing System</h1>
          </div>
      ...
      {/* Adjust padding only if header is visible */}
